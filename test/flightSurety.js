...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('(airline) can register only 4 airlines using registerAirline() without the need of consensus', async () => {
    ...
    let resultIsAirlineRegistered3 = await config.flightSuretyData.isAirlineRegistered.call(thirdAirline);
    let resultIsAirlineRegistered4 = await config.flightSuretyData.isAirlineRegistered.call(fourthAirline);
    let resultIsAirlineRegistered5 = await config.flightSuretyData.isAirlineRegistered.call(fifthAirline);
    // ASSERT
    ...
    assert.equal(await config.flightSuretyData.getAirlineCount(), 4);
  });
  ...
  it('(airline) can register another airline with at least 50% of consensus', async () => {
    ...
    assert.equal(await config.flightSuretyData.getAirlineCount(), 5);
  });
  ...
  it('(passengers) receive credit for the insurance bought and only once', async () => {
    ...
    await config.flightSuretyData.creditInsurees(config.firstAirline, airlineName, timestamp, {
        from: config.owner
    });
    await expectThrow(config.flightSuretyData.creditInsurees(config.firstAirline, airlineName, timestamp, {
        from: config.owner
    }));
    ...
    let resultPassengerAmountCredit2 = await config.flightSuretyData.getInsureeCredits.call(passengerAccount2, {
        from: passengerAccount2
    });
